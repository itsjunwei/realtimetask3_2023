...
def train_epoch(data_generator, optimizer, model, criterion, params, device, scheduler=None,
    ...
    for data, target in track(data_generator.generate(), description="Training...",
        ...
        if apply_augmentations:
            ...
            transformations = ComposeTransformNp([
                RandomShiftUpDownNp(),
                CompositeCutout(image_aspect_ratio=float(data.shape[2] / data.shape[3]), # n_timesteps / n_features
                                n_zero_channels=3),
            ])

            # Loop through the batch and apply augmentations to each data sample of the batch
...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            if epoch_cnt/nb_epoch > 0.95:
                apply_aug = False
            else:
                apply_aug = params['use_augmentations']
            
            train_loss = train_epoch(data_gen_train, optimizer, model, criterion, params, device, scheduler=scheduler,
                                     apply_augmentations=apply_aug)
            train_time = time.time() - start_time

            # ---------------------------------------------------------------------
            # VALIDATION
            # ---------------------------------------------------------------------
            if (epoch_cnt > int(0.8 * nb_epoch)) or (epoch_cnt % 5 == 0) or params['finetune_mode'] is True:
                start_time = time.time()
