...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self._normalized_spec = params['normalize_specs']
        ...
        if self._use_salsalite and self._dataset=='mic':
            ...
            assert self._upper_bin <= self._cutoff_bin, 'Upper bin for doa feature {} is higher than cutoff bin for spectrogram {}!'.format()
    ...
    def preprocess_features(self):
        ...
        if self._is_eval:
            ...
            if self._normalized_spec:
                print("Normalizing 4 channel spectrograms")
                scaler_dict = {}
                ...
                for file_cnt, file_name in enumerate(os.listdir(self._feat_dir)):
                    ...
                    for i_chan in np.arange(4):
                        scaler_dict[i_chan].partial_fit(feat_file[i_chan, : , :])
                ...
                scaler_path = normalized_features_wts_file + ".h5"
        ...
        for file_cnt, file_name in enumerate(os.listdir(self._feat_dir)):
            ...
            feat_file = np.load(os.path.join(self._feat_dir, file_name))
            if self._normalized_spec:
                feat_file[:4] = (feat_file[:4] - feature_mean)/feature_std
                feat_file = feat_file.transpose((1, 0, 2))
                feat_file = feat_file.reshape(feat_file.shape[0], -1)
